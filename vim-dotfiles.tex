...
\key{Highlight current search term}{{\leader}hs}
}

\vskip 10pt

\vbox{\head{Version Control}
\key{Show diff from latest revision}{{\leader}vd}
\key{Commit changes}{{\leader}vc}
\key{Show change log}{{\leader}vl}
\key{Status}{{\leader}vs}
}
