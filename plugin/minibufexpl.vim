...
function! <SID>BuildBufferList(delBufNum, updateBufList)
  ...
  while(l:i <= l:NBuffers)
    ...
    if (a:delBufNum == -1 || l:i != a:delBufNum)
      ...
      if(getbufvar(l:i, '&buflisted') == 1)
        ...
        " Get the long name of this buffer.
        let l:longBufName = fnamemodify(bufname(l:i), ":p")
        ...
        if(strlen(l:BufName))
          ...
          if (getbufvar(l:i, '&modifiable') == 1 && BufName != '-MiniBufExplorer-')
            ...
            " if bufwinnr(l:i) != -1
            " If the buffer we're going to is this buffer, 
            " mark it.  Don't care about other buffers open in other
            call <SID>DEBUG('Current Buffer Being Examined is: '. l:longBufName,10)
            call <SID>DEBUG('Current Buffer is: '. g:longNameCurrentBuffer, 10)
            " NJN
            if l:longBufName == g:longNameCurrentBuffer
            call <SID>DEBUG('Setting star for: '. g:longNameCurrentBuffer, 10)
              let l:tab = l:tab . '*'
            endif
  ...
  "           call <SID>DEBUG('mini buf list is different than l file names'. g:longNameCurrentBuffer, 10)
  "   if (a:updateBufList)
  "           call <SID>DEBUG('updatebufilst is true'. g:longNameCurrentBuffer, 10)
  let g:miniBufExplBufList = l:fileNames
  let s:maxTabWidth = l:maxTabWidth
  "   endif
  "   return 1
  " else
  "           call <SID>DEBUG('updatebufilst is NOT true'. g:longNameCurrentBuffer, 10)
  "   return 0
  " endif
  return 1
endfunction
...
function! <SID>AutoUpdate(delBufNum)
  ...
  call <SID>DEBUG('===========================',10)
  " Get the name of the buffer we're in.
  let g:longNameCurrentBuffer = fnamemodify(bufname("%"), ":p")
